...
public class BossCombatManager : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        if (MainManager.instance.difficulty == 1){
            GameObject.FindWithTag("1").GetComponent<EnemyManager>().enabled = false;
            GameObject.FindWithTag("1").SetActive(false);
            GameObject.FindWithTag("ehp1").SetActive(false);
            GameObject.FindWithTag("2").GetComponent<EnemyManager>().enabled = false;
    ...
    void Start()
    {
        if (GameObject.FindWithTag("0") != null)
        {
            ...
            enemy1Anim = GameObject.FindWithTag("0").GetComponent<Animator>();
        }
   ...
   void Update()
        ...
        if (myTurn)
        {
            ...
            if(playerAnim.GetCurrentAnimatorStateInfo(0).IsName("attack")){
                ...
                if(checkWin() == enemies.Count){
                    ...
                    BossCombatUI.instance.refreshUI();
                    BossCombatUI.instance.win();
                }
            }
            if(AP < baseAPCost){
                ...
                BossCombatUI.instance.endTurn.style.display = DisplayStyle.Flex;
            }
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (hit.collider != null)
            {
                ...
                if(Action(abilitySelect)){
                    ...
                    BossCombatUI.instance.SelectText.SetActive(false);
                    BossCombatUI.instance.refreshUI();
                    if(target.tag == "0"){
                        HitAnimCall(enemy1Anim);  
                    }
    ...
    public void enemyTurn()
    {
        ...
        foreach (GameObject enemy in enemies)
        {
              if (enemy.GetComponent<EnemyManager>().type == "boss" && enemy.GetComponent<EnemyManager>().enabled == true)
            {
                DMG = enemy.GetComponent<EnemyManager>().BossTurn();
                if(defend){
